# ☁️ Arrow English 구글 드라이브(Google Drive) 동기화 가이드

안녕하세요! Arrow English 학습자 여러분의 소중한 학습 데이터를 안전하게 보관하고, 여러 기기(데스크톱, 노트북 등)에서 자유롭게 이어 하실 수 있도록 **구글 드라이브 동기화 기능**을 제공하고 있습니다.
...
```mermaid
graph TD
    A[Arrow English 웹 앱] -- "1. 파일 생성/수정 (.json)" --> B["PC 로컬 구글 드라이브 폴더 (예: G:\내 드라이브\ArrowEnglish)"]
    B -- "2. 실시간 클라우드 업로드" --> C[구글 드라이브 클라우드 서버]
    C -- "3. 노트북 등 타 기기로 다운로드" --> D["다른 PC의 구글 드라이브 폴더"]
    D -- "4. 클릭 한 번으로 불러오기" --> E[노트북 Arrow English 웹 앱]
```
...
2. 마우스 우클릭을 하여 **새 폴더**를 생성하고, 이름을 **`ArrowEnglish`**로 지정합니다.
   * *경로 예시: `G:\내 드라이브\ArrowEnglish` 또는 `H:\내 드라이브\ArrowEnglish`*

---

### 3단계: Arrow English 프로그램에서 폴더 연동하기
이제 프로그램과 방금 만든 구글 드라이브 폴더를 연결합니다.

1. **Arrow English 프로그램**을 실행합니다.
...
5. 브라우저의 폴더 선택 창이 뜨면, **`Google Drive (G: 또는 H:)`** ➔ **`내 드라이브`** ➔ **`ArrowEnglish`** 폴더를 선택하고 **`폴더 선택`** 버튼을 누릅니다.
6. 브라우저 상단에 **"이 사이트에서 파일 수정을 허용하시겠습니까?"**라는 권한 요청 창이 뜨면 반드시 **`파일 보기 및 변경` (또는 `허용`/`예`)**을 선택해 줍니다.
7. 완료되면 화면에 **"☁️ Google Drive / 클라우드 폴더 [ArrowEnglish]와 자동 동기화되었습니다!"**라는 알림이 표시됩니다.
...
> 이때는 당황하지 마시고 **`⚡ 기존 폴더 [ArrowEnglish] 연동 승인하기`** 버튼을 누른 후, 브라우저 상단의 **`허용`** 버튼을 한 번만 클릭해 주시면 즉시 데이터가 복원됩니다.

### Q2. 노트북과 데스크톱 양쪽에서 데이터를 같이 쓰고 싶습니다. 어떻게 하나요?
> [!IMPORTANT]
> 1. 양쪽 PC 모두 **PC용 구글 드라이브 앱**이 설치되어 로그인되어 있어야 합니다.
> 2. 양쪽 PC에서 Arrow English 프로그램 접속 후, 똑같이 구글 드라이브 안의 **`ArrowEnglish` 폴더**를 선택해 줍니다.
